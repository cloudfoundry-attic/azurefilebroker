...
var _ = Describe("FileStore", func() {
	var (
		...
		logger     lager.Logger
		state      azurefilebroker.DynamicState
	)

	BeforeEach(func() {
		logger = lagertest.NewTestLogger("test-broker")
		fakeIoutil = &ioutil_fake.FakeIoutil{}
		store = azurefilebroker.NewFileStore("/tmp/whatever", fakeIoutil)
		state = azurefilebroker.DynamicState{
			InstanceMap: map[string]azurefilebroker.ServiceInstance{
				"service-name": {
	...
	Describe("Create, Retrieve and Delete InstanceDetails", func() {
		var (
			instanceID         string
			err                error
			outInstanceDetails azurefilebroker.ServiceInstance
			inInstanceDetails  azurefilebroker.ServiceInstance
		)
		JustBeforeEach(func() {
		...
		Context("when details found", func() {
			BeforeEach(func() {
				...
				inInstanceDetails = azurefilebroker.ServiceInstance{ServiceID: "sample-service"}
			...
			It("reports conflicts correctly", func() {
				...
				otherInstance := azurefilebroker.ServiceInstance{ServiceID: "sample-service", PlanID: "foo"}
